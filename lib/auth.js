...
if (!process.env.JWT_SECRET && process.env.NODE_ENV === 'production') {
  console.error('CRITICAL: JWT_SECRET is not set in production environment variables! Using default is unsafe.');
}

export const JWT_SECRET = process.env.JWT_SECRET || 'koyyam_markaz_dev_jwt_key_not_for_prod';
...
export function verifyAuth(req) {
  ...
  // If not in Authorization header, check HttpOnly session cookie
  if (!token && req.cookies && typeof req.cookies.get === 'function') {
    const cookieToken = req.cookies.get('markaz_token');
    token = cookieToken?.value || (typeof cookieToken === 'string' ? cookieToken : null);
  }
